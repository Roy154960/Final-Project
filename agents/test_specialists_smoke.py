...
async def test_retrieval_qa_node_full_react_loop():
    ...
    CITED_ANSWER = "The grounded answer about glazing. [cennini_libro_dell_arte.pdf]"
    ...
    # The final scripted AIMessage is a DELIBERATE paraphrase with the
    # citation stripped -- reproducing the exact confirmed failure mode
    # (see specialists.py's _extract_grounded_answer docstring: a live
    # run showed llama3.2 doing exactly this despite an explicit
    # instruction not to). If retrieval_qa_node ever regresses to using
    # result["messages"][-1].content again instead of
    # _extract_grounded_answer(), this test must fail.
    llm = ScriptedChatModel(responses=[
        ...
        AIMessage(content="", tool_calls=[{"name": "generate_answer",
                                            ...
                                            "id": str(uuid.uuid4())}]),
        AIMessage(content="Glazing is a layering technique used by painters."),  # no citation, on purpose
    ])
    ...
    final_turn = result["messages"][-1].content
    extracted = specialists._extract_grounded_answer(result["messages"])
    ...
    _check("the scripted final turn really did strip the citation (sanity-check the test itself)",
           "cennini_libro_dell_arte.pdf" not in final_turn)
    _check("_extract_grounded_answer recovers generate_answer's cited output, not the paraphrase",
           extracted == CITED_ANSWER)
...
async def test_build_specialists_wiring():
    ...
    _check(
        "build_specialists() returns all eight known route names "
        "(original three plus image_qa, painting_lookup, product_search, "
        "invoice, color_palette)",
        set(node_map.keys())
        == {
            "retrieval_qa",
            "corpus_meta",
