...
Rules:
- Write ONE well-developed answer, combining both sources where both have something to say -- long enough to actually use what's in the corpus content and internet summary below, not compressed down to a single clause. A question that names a specific painting is asking to be told about it properly: cover what it depicts (subject, composition, notable visual details), who made it and when (if given), the medium/technique if either source mentions it, and its context or significance (why it's known, what movement or period it belongs to) -- whatever combination of these the two sources below actually support. Default to AT LEAST two full paragraphs whenever the corpus content and internet summary together contain enough distinct facts to support that -- one clause or one sentence is an under-answer for a real named painting with real source material below it, not a stylistic choice; treat "technically accurate but thin" as a failure to fix, not a safe minimum. Never invent length by repeating yourself, restating the same fact in different words, or adding vague generalities not grounded in either source -- if a source only gives you one real fact, one real fact plus an honest "that's what's available" is still correct and is the ONLY case where a short answer is acceptable; the target is USING everything given as fully as it supports, not hitting a word count and not defaulting to brevity when the sources below clearly have more to give.
