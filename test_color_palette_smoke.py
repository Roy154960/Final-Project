...
Run with (from the project root -- NOT from inside agents/, and NOT as a
bare `python test_color_palette_smoke.py`; either of those leaves the
project root off sys.path, so `from agents import specialists` below
fails with "No module named 'agents'"):
    python -m agents.test_color_palette_smoke
"""
